...
display_help() {
    ...
    echo
    echo "The following argument is available only with options 'all' and getConfig "
    echo "   --disable-security-migration Disable security migration(users,groups and privileges) from Nexus Repository to Artifactory "
    echo
...
readUserInputs() {
    ...
    if [[ "$IsArtInteractive" == "true" ]]; then
      ...
      #docker run -it --name jfrog-nexus-migrator -v $JFROG_HOME:/root/.jfrog $IMAGE_NAME:$IMAGE_TAG jfrog c add --insecure-tls=true
      jfrog c add
    else
      #docker run -it --name jfrog-nexus-migrator -v $JFROG_HOME:/root/.jfrog $IMAGE_NAME:$IMAGE_TAG jfrog c add artifactory --url="${artifactoryUrl}" --user="${artifactoryUser}" --password="${artifactoryPass}" --interactive=false --insecure-tls=true
      jfrog c rm artifactory --quiet
      jfrog c add artifactory --url="${artifactoryUrl}" --user="${artifactoryUser}" --password="${artifactoryPass}" --interactive=false
    fi
    cp -f  /root/.jfrog/jfrog-cli.conf.v6 /root/.jfrog/jfrog-cli.conf.v5
    check_command "[ERROR] : Failed to set Artifactory with JFrog CLI"
    SERVER_ID=$(cat $JFROG_HOME/jfrog-cli.conf.v5 | grep "serverId" | awk -F'"serverId":' '{print $2}' | awk -F'"' '{print $2}')
...
docker_run() {
  ...
  if [ $command == "sc" ] || [ $command == "all" ]; then
    ...
    if [ $command == "all" ]; then
      if [ "$disableSecurity" == "true" ];then
        extra_args="$extra_args --disable-security-migration"
      fi
    fi
    ...
    if [ $command == "ma" ]; then
      readTerminalInput "$@"
      if [ "$repoList" != "" ]; then
          extra_args="$extra_args --repos=$repoList"
      fi
 ...
 # docker_container_check
 # docker run -i --name jfrog-nexus-migrator -e migratorLogLevel=${MIGRATOR_LOG_LEVEL} -e ENABLE_ASSET_PLUGIN="${ENABLE_ASSET_PLUGIN}" -e SKIP_CHECKSUM_INTEGRITY_CHECK="${SKIP_CHECKSUM_INTEGRITY_CHECK}" -v $JFROG_HOME:/root/.jfrog -v "$NEXUS_DATA":"$NEXUS_DATA" $IMAGE_NAME:$IMAGE_TAG jfrog-nexus-migrator $command $extra_args >> migration.log 2>&1 &
